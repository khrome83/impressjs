...
var createContext = function(node, props, options, value, key, data) {
    return {
        ...
        options: options,
        value: value || '',
        key: key || ('defaults' in props && 'key' in props.defaults) ? props.defaults.key : null,
        data: data || {}
    };
};
...
exports.setup = function(manifest) {
    ...
    if(manifest.element && Object.keys(manifest.element).length > 0) {
        // Parser for Element
        _parsers['element'] = parseElement;
    }
    
    // Check if we have ID plugins
    if(manifest.id && Object.keys(manifest.id).length > 0) {
        // Parser for Id
        _parsers['id'] = parseId;
    }
    
    // Check if we have Class plugins
    if(manifest.class && Object.keys(manifest.class).length > 0) {
        // Parser for Class
        _parsers['class'] = parseClasses;
    }
    
    // Check if we have Attribute plugins
    if(manifest.attribute && Object.keys(manifest.attribute).length > 0) {
