 ...
 */
function updateManifest(plugins, prefix) {
    var output = actions.createManifest(plugins, prefix, {records: records, adapter: adapter});
    parser.setup(output);
...
exports.compile = function (html, data) {    
    var options = {
        ...
        tree = (/^\s*<(!doctype|html|head|body)\b/i.test(html)) ? parse5.parse(html, options) : parse5.parseFragment(html, options),
        output = parser.parse(tree, _manifest, _options, data);
